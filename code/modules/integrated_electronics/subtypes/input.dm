...
/obj/item/integrated_circuit/input/med_scanner/do_work()
	...
	if(H.Adjacent(get_turf(src))) // Like normal analysers, it can't be used at range.
		...
		set_pin_data(IC_OUTPUT, 2, H.get_pulse())
...
/obj/item/integrated_circuit/input/slime_scanner/do_work()
	...
	if(T in view(get_turf(src))) // Like medbot's analyzer it can be used in range..
		...
		//set_pin_data(IC_OUTPUT, 6, T.GetMutations())		//Different xeno system I think???
		set_pin_data(IC_OUTPUT, 6, T.mutation_chance)
		set_pin_data(IC_OUTPUT, 7, T.cores)
		set_pin_data(IC_OUTPUT, 8, T.amount_grown/SLIME_EVOLUTION_THRESHOLD)
...
/obj/item/integrated_circuit/input/microphone/hear_talk(var/mob/living/M as mob, text, verb, datum/language/speaking)
	...
	push_data()
	activate_pin(1)
	if(translated && !(speaking.name == LANGUAGE_COMMON))
		activate_pin(2)
