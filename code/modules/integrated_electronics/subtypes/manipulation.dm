...
/obj/item/integrated_circuit/manipulation/plant_module/do_work()
	...
	if(istype(OM,/obj/seed) && check_target(OM) && get_pin_data(IC_INPUT, 2) == 2)
		qdel(OM)
		push_data()
		activate_pin(2)
		return
...
/proc/get_random_turf_in_range(var/atom/origin, var/outer_range, var/inner_range)
	...
	for(var/turf/T in orange(origin, outer_range))
		if(!(T.z in GLOB.maps_data.sealed_levels)) // Picking a turf outside the map edge isn't recommended
