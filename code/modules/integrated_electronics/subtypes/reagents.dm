...
/obj/item/integrated_circuit/reagent/injector/proc/inject_after(var/weakref/target)
	...
	log_admin("[key_name(L)] was successfully injected with " + reagents.update_total() + " by \the [acting_object]")
...
/obj/item/integrated_circuit/reagent/injector/proc/inject()
	...
	if(direction_mode == IC_REAGENTS_INJECT)
		...
		if(isliving(AM))
			...
			log_admin("[key_name(L)] is getting injected with " + reagents.update_total() + " by \the [acting_object]")
...
/obj/item/integrated_circuit/reagent/storage/cryo
	...
	flags = NO_TEMP_CHANGE | OPENCONTAINER | NO_REACT
	complexity = 8
	spawn_flags = IC_SPAWN_RESEARCH
...
/obj/item/integrated_circuit/reagent/filter/do_work()
	...
	for(var/datum/reagent/G in source.reagents.reagent_list)
		if(!direction_mode)
			if(G.name in demand)
				source.reagents.trans_type_to(target, G.type, possible_transfer_amounts)
		else
			if(!(G.name in demand))
				source.reagents.trans_type_to(target, G.type, possible_transfer_amounts)
	activate_pin(2)
...
/obj/item/integrated_circuit/reagent/temp
	...
	var/temperature = 0
	var/active = 0
	var/min_temp = 40 //40 CELSIUS
	var/max_temp = 200 //200 CELSIUS
	var/heating_power = 5
...
/obj/item/integrated_circuit/reagent/temp/cooler
	...
	min_temp = -80 //-80 CELSIUS
	max_temp = 30 //30 CELSIUS
